        ...
        def process(self, in_msg):
            if in_msg.getMsgType() == TransistorMsg.MsgType.MsgType_Command:
                if in_msg.getCommand() == TransistorMsg.Command.Command_Potentiometer:
                        if in_msg.getPotentiometer() == TransistorMsg.Potentiometer.Potentiometer_Volume:
                            self.processPotentiometerVolume(in_msg.getPotentiometerValue())
                        elif in_msg.getPotentiometer() == TransistorMsg.Potentiometer.Potentiometer_Tuner:
                ...
                if in_msg.getSystem() == TransistorMsg.System.System_SendBatteryStatus:
                        self.processSendBatteryStatus(in_msg.getSendBatteryStatusPercentage(), in_msg.getSendBatteryStatusCharging())


                elif in_msg.getSystem() == TransistorMsg.System.System_SendBatteryVoltage:
                        self.processSendBatteryVoltage(in_msg.getSendBatteryVoltageVoltage())
